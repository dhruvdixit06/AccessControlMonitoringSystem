...
└── README.md
Application Flow
Login: User selects a role (simulated authentication).
Dashboard: User lands on the dashboard corresponding to their role (e.g., Admin Dashboard).
Action: User performs actions (e.g., Add User, Review Access) which trigger API calls to the backend.
Persistence: Backend processes requests using Services and updates the SQLite database.
